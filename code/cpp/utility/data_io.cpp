...
namespace IMUProject{
    ...
    IMUDataset::IMUDataset(const std::string &directory, unsigned char load_control): file_io_(directory) {
        ...
        timestamp_.convertTo(timestamp_, CV_64FC1);

        double* ts = (double*) timestamp_.data;
        for(int i=0; i<timestamp_.rows; ++i){
            ts[i] /= kNanoToSec;
        }
        if (load_control & IMU_ORIENTATION) {
            orientation_ = data_mat.colRange(layout_.orientation, layout_.orientation + 4).clone();
            orientation_.convertTo(orientation_, CV_64FC1);
        ...
        if(load_control & IMU_ACCELEROMETER){
            ...
            accelerometer_.convertTo(accelerometer_, CV_64FC1);
        ...
        if(load_control & IMU_LINEAR_ACCELERATION){
            ...
            linear_acceleration_.convertTo(linear_acceleration_, CV_64FC1);
        ...
        if(load_control & IMU_GRAVITY){
            ...
            gravity_.convertTo(gravity_, CV_64FC1);
