...
namespace IMUProject{
    ...
    void WriteToPly(const std::string& path, const cv::Mat& position, const cv::Mat& orientation,
        ...
        CHECK_EQ(position.type(), CV_32FC1);
        CHECK_EQ(orientation.type(), CV_32FC1);
	    CHECK_EQ(position.rows, orientation.rows);
        CHECK_EQ(position.cols, 3);
	    ...
	    for (int i = 0; i < position.rows; ++i) {
		    const float *pt = (float *) position.ptr(i);
	    ...
	    if (kpoints > 0 && interval > 0 && axis_length > 0) {
		    Eigen::Matrix3f local_axis = Eigen::Matrix3f::Identity();
		    for (int i = 0; i < position.rows; i += interval) {
			    const float *ori_ptr = (float *) orientation.ptr(i);
			    const float *pos_ptr = (float *) position.ptr(i);
			    Eigen::Quaternionf q(ori_ptr[0], ori_ptr[1], ori_ptr[2], ori_ptr[3]);
			    Eigen::Matrix3f axis_dir = q.toRotationMatrix() * local_axis;
			    Eigen::Vector3f pos(pos_ptr[0], pos_ptr[1], pos_ptr[2]);
			    for (int j = 0; j < kpoints; ++j) {
				    for(int k=0; k<3; ++k){
					    Eigen::Vector3f pt = pos + axis_length / kpoints * j * axis_dir.block<3,1>(0, k);
					    TriMesh::VertexHandle vertex = mesh.add_vertex(TriMesh::Point(pt[0], pt[1], pt[2]));
